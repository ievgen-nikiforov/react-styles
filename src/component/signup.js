import React, { Component } from 'react';
class Signup extends Component {
    state = {
        Checkbox: true, 
        email: "",
        password: "",
        name : "",

    }

    changeData = (event) => {
        if (event.target.name ==="checkbox"){
            this.setState({
                iCheckbox: !this.state.isChecked,
            },
            () => {
                console.log(this.state);
              }
            );   
        }
        else{
        this.setState({            
        [event.target.name]: event.target.value,
        },
        () => {
            console.log(this.state);
          }
       
        )}
        
    }

render() {

    return (
  <div className="sign_content">
      <div className= "lock_icon">
      <img className = "lock_icon_img" src="data:image/svg+xml;base64," alt = "lock icon" />
      </div>
      <div className="signup_text">Sign up</div>
      <form>
      <div className = "input input-name-surname"><input className = "input_field_name" name="name" value={this.state.name} onChange={this.changeData} placeholder="First Name*"/>
    
      <input className = "input_field_surname" name="surname" value={this.state.surname} onChange={this.changeData} placeholder="Last Name*"/>
        </div> 

      <div className = "input"><input className = "input_field" name="email" value={this.state.email} onChange={this.changeData} placeholder="Email Adress*"/>
        </div>
        <div className = "input"><input className = "input_field" name="password" value={this.state.password} onChange={this.changeData} placeholder="Password*"/>
        </div>
        <div className = "checkbox_block">
        <input type="checkbox" name="subscribe"  onChange={this.changeData} defaultChecked={this.state.Checkbox} className = "checkbox"/> 
        <p className = "subscribe">I want to recieve inspiration, marketing promotions and updates via email</p>
        </div>
        <input type="submit" className="submit_button" value="SIGN IN" />
        </form>
        <div className = "forgot_signin">
        <a href="#/signin" className="have_account">Already have an account? Sign in</a>
        </div>
        <div className="copyright">Copyright © Your Website 2020.</div>
      </div>
    );
}
}


export default Signup;